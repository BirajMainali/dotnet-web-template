...
(function ($) {
    var calendarData = {
        bsMonths: ["Baishak", "Jestha", "Ashad", "Shrawan", "Bhadra", "Ashoj", "Kartik", "Mangshir", "Poush", "Magh", "Falgun", "Chaitra"],
    ...
    $.extend(calendarFunctions, {
        ...
        getBsDateByAdDate: function (adYear, adMonth, adDate) {
            ...
            else if (adMonth == 4 && yearDecreased && adDate >= 1 && adDate <= bsMonthFirstAdDate.getDate()) {
                bsMonth = 12;
                var bsMonthDays = calendarFunctions.getBsMonthDays(bsYear, bsMonth);
                bsDate = bsMonthDays - (bsMonthFirstAdDate.getDate() - adDate) + 1;
            }
            ...
            const newAdYear = bsMonthFirstAdDate.getFullYear();
            if (adMonth === 4 && bsMonth === 12 && ((newAdYear % 4 === 0) && (newAdYear % 100 !== 0)) || (newAdYear % 400 === 0) ) {
                bsDate -= 1
            }
        ...
        validateNepDate: function (bsYear, bsMonth, bsDate) {
            if (typeof bsDate !== "number" || bsDate === null) {
                throw new TypeError("Invalid date value");
            } else if (bsDate < 1 || bsDate > 32) {
                throw new RangeError("Date value should be in range of 1 to 32");
            } else if (bsDate > calendarFunctions.getBsMonthDays(bsYear, bsMonth)) {
                throw new RangeError("Date value should be in range of given month & year");
            }
            // else if (typeof bsMonth !== "number" || bsMonth === null) {
            //    throw new TypeError("Invalid month value");
            //} else if (bsMonth < 1 || bsMonth > 12) {
            //    throw new RangeError("Month value should be in range of 1 to 12");
            //} else if (typeof bsYear !== "number" || bsYear === null) {
            //    throw new TypeError("Invalid year value");
            //} else if (bsYear < calendarData.minBsYear || bsYear > calendarData.maxBsYear) {
            //    throw new RangeError("Year value should be in range of " + calendarData.minBsYear + " to " + calendarData.maxBsYear);
            //}
        },
    });
